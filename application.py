...
# Configure app to use SQLAlchemy database
...
def location():
    ...
    location_businesses = db.execute("SELECT * FROM location_businesses WHERE location = :location", location=str_location).fetchall()
    ...
    # look up businesses for this location
        ...
        db.execute("INSERT INTO location_businesses (location, businesses) VALUES (:location, :businesses)", 
                    {"location": str_location, "businesses": str_location_businesses})
...
def businesses():
    ...
    # if business details are in database, no need to look up on yelp
    business_details = db.execute("SELECT * FROM businesses WHERE location = :location", location=location).fetchall()
    if len(business_details) >= 1:
        eprint("we have details for location")
